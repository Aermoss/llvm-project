...
//===----------------------------------------------------------------------===//
//
// This file implements the ZodiacMCCodeEmitter class.
//
//===----------------------------------------------------------------------===//
...
static Zodiac::Fixups FixupKind(const MCExpr *Expr) {
  ...
  if (const MCSpecifierExpr *McExpr = dyn_cast<MCSpecifierExpr>(Expr)) {
    ...
    case Zodiac::S_ABS_HI:
      return Zodiac::FIXUP_ZODIAC_HI16;
    case Zodiac::S_ABS_LO:
      return Zodiac::FIXUP_ZODIAC_LO16;
    }
  }
...
static unsigned adjustPqBits(const MCInst &Inst, unsigned Value,
  ...
  if (!LPAC::isPostOp(AluCode) &&
      ((Op2.isImm() && Op2.getImm() != 0) ||
       (Op2.isReg() && Op2.getReg() != Zodiac::R0) || (Op2.isExpr())))
    Value |= (1 << PBitShift);
  ...
  if (LPAC::modifiesOp(AluCode) && ((Op2.isImm() && Op2.getImm() != 0) ||
                                    (Op2.isReg() && Op2.getReg() != Zodiac::R0)))
    Value |= (1 << QBitShift);
...
unsigned
ZodiacMCCodeEmitter::adjustPqBitsRmAndRrm(const MCInst &Inst, unsigned Value,
                                         const MCSubtargetInfo &STI) const {
  return adjustPqBits(Inst, Value, 17, 16);
}

unsigned
ZodiacMCCodeEmitter::adjustPqBitsSpls(const MCInst &Inst, unsigned Value,
                                     const MCSubtargetInfo &STI) const {
...
// Encode Zodiac Memory Operand
unsigned ZodiacMCCodeEmitter::getRiMemoryOpValue(
  ...
  Encoding = (getZodiacRegisterNumbering(Op1.getReg()) << 18);
  if (Op2.isImm()) {
...
unsigned ZodiacMCCodeEmitter::getRrMemoryOpValue(
    const MCInst &Inst, unsigned OpNo, SmallVectorImpl<MCFixup> &Fixups,
    const MCSubtargetInfo &SubtargetInfo) const {
  unsigned Encoding;
  const MCOperand Op1 = Inst.getOperand(OpNo + 0);
  ...
  assert(Op1.isReg() && "First operand is not register.");
  Encoding = (getZodiacRegisterNumbering(Op1.getReg()) << 15);
  assert(Op2.isReg() && "Second operand is not register.");
  Encoding |= (getZodiacRegisterNumbering(Op2.getReg()) << 10);

  assert(AluMCOp.isImm() && "Third operator is not immediate.");
  // Set BBB
  unsigned AluOp = AluMCOp.getImm();
  Encoding |= LPAC::encodeZodiacAluCode(AluOp) << 5;
  // Set P and Q
...
unsigned
ZodiacMCCodeEmitter::getSplsOpValue(const MCInst &Inst, unsigned OpNo,
                                   SmallVectorImpl<MCFixup> &Fixups,
                                   const MCSubtargetInfo &SubtargetInfo) const {
  unsigned Encoding;
  const MCOperand Op1 = Inst.getOperand(OpNo + 0);
  ...
  Encoding = (getZodiacRegisterNumbering(Op1.getReg()) << 12);
  if (Op2.isImm()) {
...
unsigned ZodiacMCCodeEmitter::getBranchTargetOpValue(
    const MCInst &Inst, unsigned OpNo, SmallVectorImpl<MCFixup> &Fixups,
    const MCSubtargetInfo &SubtargetInfo) const {
  ...
  Fixups.push_back(MCFixup::create(0, MCOp.getExpr(), Zodiac::FIXUP_ZODIAC_25));
