...
//===----------------------------------------------------------------------===//
//
// This file defines an instruction selector for the Zodiac target.
//
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// ZodiacDAGToDAGISel - Zodiac specific code to select Zodiac machine
...
// Helper functions for ComplexPattern used on ZodiacInstrInfo
// Used on Zodiac Load/Store instructions.
bool ZodiacDAGToDAGISel::selectAddrSls(SDValue Addr, SDValue &Offset) {
  ...
  if (Addr.getOpcode() == ISD::OR &&
      Addr.getOperand(1).getOpcode() == ZodiacISD::SMALL) {
...
bool ZodiacDAGToDAGISel::selectAddrRiSpls(SDValue Addr, SDValue &Base,
  ...
  if (ConstantSDNode *CN = dyn_cast<ConstantSDNode>(Addr)) {
    if (RiMode) {
      ...
      if (isInt<16>(CN->getSExtValue())) {
        ...
        Offset = CurDAG->getTargetConstant(Imm, DL, CN->getValueType(0));
        Base = CurDAG->getRegister(Zodiac::R0, CN->getValueType(0));
...
bool ZodiacDAGToDAGISel::selectAddrRi(SDValue Addr, SDValue &Base,
                                     SDValue &Offset, SDValue &AluOp) {
  return selectAddrRiSpls(Addr, Base, Offset, AluOp, /*RiMode=*/true);
}

bool ZodiacDAGToDAGISel::selectAddrSpls(SDValue Addr, SDValue &Base,
                                       SDValue &Offset, SDValue &AluOp) {
...
namespace llvm {
namespace LPAC {
static AluCode isdToZodiacAluCode(ISD::NodeType Node_type) {
  switch (Node_type) {
  case ISD::ADD:
    return AluCode::ADD;
  case ISD::ADDE:
...
bool ZodiacDAGToDAGISel::selectAddrRr(SDValue Addr, SDValue &R1, SDValue &R2,
                                     SDValue &AluOp) {
  // if Address is FI, get the TargetFrameIndex.
  ...
  ISD::NodeType AluOperator = static_cast<ISD::NodeType>(Addr.getOpcode());
  LPAC::AluCode AluCode = LPAC::isdToZodiacAluCode(AluOperator);
  if (AluCode != LPAC::UNKNOWN) {
    ...
    if (Addr.getOperand(0).getOpcode() == ZodiacISD::HI ||
        Addr.getOperand(0).getOpcode() == ZodiacISD::LO ||
        Addr.getOperand(0).getOpcode() == ZodiacISD::SMALL ||
        Addr.getOperand(1).getOpcode() == ZodiacISD::HI ||
        Addr.getOperand(1).getOpcode() == ZodiacISD::LO ||
        Addr.getOperand(1).getOpcode() == ZodiacISD::SMALL)
      return false;
