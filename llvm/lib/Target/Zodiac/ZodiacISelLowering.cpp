...
//===----------------------------------------------------------------------===//
//
// This file implements the ZodiacTargetLowering class.
//
//===----------------------------------------------------------------------===//
...
static cl::opt<int> ZodiacLowerConstantMulThreshold(
    "zodiac-constant-mul-threshold", cl::Hidden,
...
ZodiacTargetLowering::ZodiacTargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(Zodiac::SP);
...
SDValue ZodiacTargetLowering::LowerOperation(SDValue Op,
...
//===----------------------------------------------------------------------===//
//                       Zodiac Inline Assembly Support
//===----------------------------------------------------------------------===//
...
static bool CC_Zodiac32_VarArg(unsigned ValNo, MVT ValVT, MVT LocVT,
  ...
  if (!ArgFlags.isVarArg())
    return CC_Zodiac32(ValNo, ValVT, LocVT, LocInfo, ArgFlags, OrigTy, State);
...
SDValue ZodiacTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // Zodiac target does not yet support tail call optimization.
  IsTailCall = false;
...
SDValue ZodiacTargetLowering::LowerCCCArguments(
  ...
  if (CallConv == CallingConv::Fast) {
    CCInfo.AnalyzeFormalArguments(Ins, CC_Zodiac32_Fast);
  } else {
  ...
  // The Zodiac ABI for returning structs by value requires that we copy
...
ZodiacTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  if (DAG.getMachineFunction().getFunction().hasStructRetAttr()) {
    ...
    Chain = DAG.getCopyToReg(Chain, DL, Zodiac::RV, Val, Glue);
    Glue = Chain.getValue(1);
    RetOps.push_back(
        DAG.getRegister(Zodiac::RV, getPointerTy(DAG.getDataLayout())));
  }
...
SDValue ZodiacTargetLowering::LowerCCCCallTo(
  ...
  if (IsVarArg) {
    CCInfo.AnalyzeCallOperands(Outs, CC_Zodiac32_VarArg);
  } else {
    if (CallConv == CallingConv::Fast)
      CCInfo.AnalyzeCallOperands(Outs, CC_Zodiac32_Fast);
    else
  ...
  for (unsigned I = 0, J = 0, E = ArgLocs.size(); I != E; ++I) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (StackPtr.getNode() == nullptr)
        StackPtr = DAG.getCopyFromReg(Chain, DL, Zodiac::SP,
  ...
  uint8_t OpFlag = ZodiacII::MO_NO_FLAG;
  if (G) {
  ...
  Chain = DAG.getNode(ZodiacISD::CALL, DL, NodeTys,
...
static LPCC::CondCode IntCondCCodeToICC(SDValue CC, const SDLoc &DL,
  ...
  // and Zodiac only supports integer comparisons, so only provide definitions
  // for them.
  switch (SetCCOpcode) {
  case ISD::SETEQ:
    return LPCC::ICC_EQ;
...
SDValue ZodiacTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue Glue = DAG.getNode(ZodiacISD::SET_FLAG, DL, MVT::Glue, LHS, RHS);

  return DAG.getNode(ZodiacISD::BR_CC, DL, Op.getValueType(), Chain, Dest,
                     TargetCC, Glue);
}

SDValue ZodiacTargetLowering::LowerMUL(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (InstrRequired > ZodiacLowerConstantMulThreshold)
    return SDValue();
...
SDValue ZodiacTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(ZodiacISD::SETCC, DL, Op.getValueType(), TargetCC, Glue);
}

SDValue ZodiacTargetLowering::LowerSELECT_CC(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
  ...
  return DAG.getNode(ZodiacISD::SELECT_CC, DL, Op.getValueType(), TrueV, FalseV,
                     TargetCC, Glue);
}
...
SDValue ZodiacTargetLowering::LowerDYNAMIC_STACKALLOC(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDValue Chain = Op.getOperand(0);
  ...
  // For Zodiac, the outgoing memory arguments area should be on top of the
  ...
  SDValue ArgAdjust = DAG.getNode(ZodiacISD::ADJDYNALLOC, DL, MVT::i32, Sub);
...
SDValue ZodiacTargetLowering::LowerRETURNADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  MachineFrameInfo &MFI = MF.getFrameInfo();
...
SDValue ZodiacTargetLowering::LowerFRAMEADDR(SDValue Op,
                                            SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
  ...
  EVT VT = Op.getValueType();
  SDLoc DL(Op);
  SDValue FrameAddr = DAG.getCopyFromReg(DAG.getEntryNode(), DL, Zodiac::FP, VT);
  unsigned Depth = Op.getConstantOperandVal(0);
  while (Depth--) {
...
SDValue ZodiacTargetLowering::LowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ConstantPoolSDNode *N = cast<ConstantPoolSDNode>(Op);
  const Constant *C = N->getConstVal();
  const ZodiacTargetObjectFile *TLOF =
      static_cast<const ZodiacTargetObjectFile *>(
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small ||
    ...
    SDValue Small = DAG.getTargetConstantPool(
        C, MVT::i32, N->getAlign(), N->getOffset(), ZodiacII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(Zodiac::R0, MVT::i32),
                       DAG.getNode(ZodiacISD::SMALL, DL, MVT::i32, Small));
  } else {
    uint8_t OpFlagHi = ZodiacII::MO_ABS_HI;
    uint8_t OpFlagLo = ZodiacII::MO_ABS_LO;
    ...
    Hi = DAG.getNode(ZodiacISD::HI, DL, MVT::i32, Hi);
    Lo = DAG.getNode(ZodiacISD::LO, DL, MVT::i32, Lo);
    SDValue Result = DAG.getNode(ISD::OR, DL, MVT::i32, Hi, Lo);
    return Result;
  }
}

SDValue ZodiacTargetLowering::LowerGlobalAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (TLOF->isGlobalInSmallSection(GO, getTargetMachine())) {
    SDValue Small = DAG.getTargetGlobalAddress(
        GV, DL, getPointerTy(DAG.getDataLayout()), Offset, ZodiacII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(Zodiac::R0, MVT::i32),
                       DAG.getNode(ZodiacISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue ZodiacTargetLowering::LowerBlockAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue ZodiacTargetLowering::LowerJumpTable(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small) {
    SDValue Small = DAG.getTargetJumpTable(
        JT->getIndex(), getPointerTy(DAG.getDataLayout()), ZodiacII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(Zodiac::R0, MVT::i32),
                       DAG.getNode(ZodiacISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue ZodiacTargetLowering::LowerSHL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  ...
  // Zodiac shifts of larger than register sizes are wrapped rather than
...
SDValue ZodiacTargetLowering::LowerSRL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  ...
  // Note: This takes advantage of Zodiac's shift behavior to avoid needing to
...
SDValue ZodiacTargetLowering::PerformDAGCombine(SDNode *N,
