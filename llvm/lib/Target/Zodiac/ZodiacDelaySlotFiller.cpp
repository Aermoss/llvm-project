//===-- ZodiacDelaySlotFiller.cpp - Zodiac delay slot filler ----------------===//
//
...
static cl::opt<bool>
    NopDelaySlotFiller("zodiac-nop-delay-filler", cl::init(false),
                       cl::desc("Fill Zodiac delay slots with NOPs."),
                       cl::Hidden);
...
struct Filler : public MachineFunctionPass {
  ...
  StringRef getPassName() const override { return "Zodiac Delay Slot Filler"; }

  bool runOnMachineBasicBlock(MachineBasicBlock &MBB);

  bool runOnMachineFunction(MachineFunction &MF) override {
    const ZodiacSubtarget &Subtarget = MF.getSubtarget<ZodiacSubtarget>();
...
// createZodiacDelaySlotFillerPass - Returns a pass that fills in delay
// slots in Zodiac MachineFunctions
FunctionPass *
llvm::createZodiacDelaySlotFillerPass(const ZodiacTargetMachine & /*tm*/) {
  return new Filler();
}
  ...
  for (MachineBasicBlock::instr_iterator I = MBB.instr_begin();
    ...
    if (I->getDesc().hasDelaySlot()) {
      ...
      if (I->getOpcode() == Zodiac::RET) {
        ...
        assert(RI->getOpcode() == Zodiac::LDW_RI && RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == Zodiac::FP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == Zodiac::FP &&
               RI->getOperand(2).isImm() && RI->getOperand(2).getImm() == -8);
        ++RI;
        assert(RI->getOpcode() == Zodiac::ADD_I_LO &&
               RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == Zodiac::SP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == Zodiac::FP);
        ...
        if (!NopDelaySlotFiller && findDelayInstr(MBB, I, J)) {
          ...
          BuildMI(MBB, std::next(I), DebugLoc(), TII->get(Zodiac::NOP));
        }
        ++FilledSlots;
      }
