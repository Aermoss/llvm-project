//===-- ZodiacAsmParser.cpp - Parse Zodiac assembly to MCInst instructions --===//
//
...
class ZodiacAsmParser : public MCTargetAsmParser {
  // Parse operands
  std::unique_ptr<ZodiacOperand> parseRegister(bool RestoreOnFailure = false);

  std::unique_ptr<ZodiacOperand> parseImmediate();

  std::unique_ptr<ZodiacOperand> parseIdentifier();
...
// ZodiacOperand - Instances of this class represented a parsed machine
// instruction
struct ZodiacOperand : public MCParsedAsmOperand {
  ...
  explicit ZodiacOperand(KindTy Kind) : Kind(Kind) {}
...
public:
  ...
  bool isHiImm16() {
    ...
    if (const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(Imm.Value))
      return SymbolRefExpr->getSpecifier() == Zodiac::S_ABS_HI;
  ...
  bool isLoImm16() {
    ...
    // Symbolic reference expression
    if (const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(Imm.Value))
      return SymbolRefExpr->getSpecifier() == Zodiac::S_ABS_LO;
  ...
  bool isLoImm21() {
    ...
    // Symbolic reference expression
    if (const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(Imm.Value))
      return SymbolRefExpr->getSpecifier() == Zodiac::S_None;
  ...
  void addLoImm16Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      ...
      assert(SymbolRefExpr && SymbolRefExpr->getSpecifier() == Zodiac::S_ABS_LO);
#endif
...
#ifndef NDEBUG
      const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(getImm());
      assert(SymbolRefExpr && SymbolRefExpr->getSpecifier() == Zodiac::S_ABS_HI);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else if (isa<MCBinaryExpr>(getImm())) {
#ifndef NDEBUG
...
#ifndef NDEBUG
      const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(getImm());
      assert(SymbolRefExpr && SymbolRefExpr->getSpecifier() == Zodiac::S_None);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
  ...
  static std::unique_ptr<ZodiacOperand> CreateToken(StringRef Str, SMLoc Start) {
  ...
  static std::unique_ptr<ZodiacOperand> createReg(MCRegister Reg, SMLoc Start,
                                                 SMLoc End) {
  ...
  static std::unique_ptr<ZodiacOperand> createImm(const MCExpr *Value,
  ...
  static std::unique_ptr<ZodiacOperand>
  MorphToMemImm(std::unique_ptr<ZodiacOperand> Op) {
  ...
  static std::unique_ptr<ZodiacOperand>
  MorphToMemRegReg(MCRegister BaseReg, std::unique_ptr<ZodiacOperand> Op,
                   unsigned AluOp) {
  ...
  static std::unique_ptr<ZodiacOperand>
  MorphToMemRegImm(MCRegister BaseReg, std::unique_ptr<ZodiacOperand> Op,
                   unsigned AluOp) {
    const MCExpr *Imm = Op->getImm();
...
bool ZodiacAsmParser::matchAndEmitInstruction(SMLoc IdLoc, unsigned &Opcode,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      ErrorLoc = ((ZodiacOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IdLoc;
    }
...
std::unique_ptr<ZodiacOperand>
ZodiacAsmParser::parseRegister(bool RestoreOnFailure) {
  ...
  if (Lexer.getKind() == AsmToken::Identifier) {
    ...
    return ZodiacOperand::createReg(Reg, Start, End);
  }
  if (PercentTok && RestoreOnFailure)
    Lexer.UnLex(*PercentTok);
  return nullptr;
...
bool ZodiacAsmParser::parseRegister(MCRegister &RegNum, SMLoc &StartLoc,
                                   SMLoc &EndLoc) {
  const AsmToken &Tok = getParser().getTok();
  StartLoc = Tok.getLoc();
  EndLoc = Tok.getEndLoc();
  std::unique_ptr<ZodiacOperand> Op = parseRegister(/*RestoreOnFailure=*/false);
  if (Op != nullptr)
    RegNum = Op->getReg();
  return (Op == nullptr);
}
...
ParseStatus ZodiacAsmParser::tryParseRegister(MCRegister &Reg, SMLoc &StartLoc,
  ...
  StartLoc = Tok.getLoc();
  EndLoc = Tok.getEndLoc();
  std::unique_ptr<ZodiacOperand> Op = parseRegister(/*RestoreOnFailure=*/true);
  if (Op == nullptr)
...
std::unique_ptr<ZodiacOperand> ZodiacAsmParser::parseIdentifier() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  const MCExpr *Res, *RHS = nullptr;
  auto Kind = Zodiac::S_None;
  ...
  return ZodiacOperand::createImm(Res, Start, End);
}

std::unique_ptr<ZodiacOperand> ZodiacAsmParser::parseImmediate() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  ...
  case AsmToken::Dot:
    if (!Parser.parseExpression(ExprVal))
      return ZodiacOperand::createImm(ExprVal, Start, End);
    [[fallthrough]];
  default:
    return nullptr;
  }
...
unsigned ZodiacAsmParser::parseAluOperator(bool PreOp, bool PostOp) {
  StringRef IdString;
  Parser.parseIdentifier(IdString);
  unsigned AluCode = LPAC::stringToZodiacAluCode(IdString);
...
bool ZodiacAsmParser::parsePrePost(StringRef Type, int *OffsetValue) {
...
bool shouldBeSls(const ZodiacOperand &Op) {
  ...
  if (const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(Op.getImm())) {
    ...
    return (LHSSymbolRefExpr &&
            LHSSymbolRefExpr->getSpecifier() == Zodiac::S_None);
  }
  return false;
}

// Matches memory operand. Returns true if error encountered.
ParseStatus ZodiacAsmParser::parseMemoryOperand(OperandVector &Operands) {
  ...
  if (Operands[0]->isToken())
    Type = static_cast<ZodiacOperand *>(Operands[0].get())->getToken();
  ...
  std::unique_ptr<ZodiacOperand> Op = parseRegister();
  if (!Op)
    Op = parseImmediate();
  ...
  Parser.Lex(); // Eat the '['.
  std::unique_ptr<ZodiacOperand> Offset = nullptr;
  if (Op)
    Offset.swap(Op);
  ...
  if (!Op) {
    if (!Offset) {
      if ((Op = parseImmediate()) && Lexer.is(AsmToken::RBrac)) {
        ...
        // SLS, the rest as RM.
        if (shouldBeSls(*Op)) {
          Operands.push_back(ZodiacOperand::MorphToMemImm(std::move(Op)));
        } else {
          if (!Op->isLoImm16Signed())
            return Error(Parser.getTok().getLoc(),
                         "Memory address is not word aligned and larger than "
                         "class RM can handle");
          Operands.push_back(ZodiacOperand::MorphToMemRegImm(
              Zodiac::R0, std::move(Op), LPAC::ADD));
        }
        return ParseStatus::Success;
      }
    }
  ...
  if (Lexer.is(AsmToken::RBrac)) {
    ...
    if (!Offset) {
      ...
      Offset = ZodiacOperand::createImm(OffsetConstExpr, Start, End);
    }
  } else {
  ...
  Operands.push_back(
      Offset->isImm()
          ? ZodiacOperand::MorphToMemRegImm(BaseReg, std::move(Offset), AluOp)
          : ZodiacOperand::MorphToMemRegReg(BaseReg, std::move(Offset), AluOp));
...
StringRef ZodiacAsmParser::splitMnemonic(StringRef Name, SMLoc NameLoc,
                                        OperandVector *Operands) {
  ...
  if (Mnemonic[0] == 'b' ||
    ...
    LPCC::CondCode CondCode =
        LPCC::suffixToZodiacCondCode(Mnemonic.substr(1, Next));
    if (CondCode != LPCC::UNKNOWN) {
      Mnemonic = Mnemonic.slice(0, 1);
      Operands->push_back(ZodiacOperand::CreateToken(Mnemonic, NameLoc));
      Operands->push_back(ZodiacOperand::createImm(
          MCConstantExpr::create(CondCode, getContext()), NameLoc, NameLoc));
      if (IsBRR) {
        Operands->push_back(ZodiacOperand::CreateToken(".r", NameLoc));
      }
      return Mnemonic;
    }
  }
  ...
  if (Mnemonic.starts_with("sel") ||
    ...
    LPCC::CondCode CondCode = LPCC::suffixToZodiacCondCode(Mnemonic);
    if (CondCode != LPCC::UNKNOWN) {
...
static bool IsMemoryAssignmentError(const OperandVector &Operands) {
  ...
  if (ZodiacOperand *PossibleAluOp =
          static_cast<ZodiacOperand *>(Operands[PossibleAluOpIdx].get()))
...
static bool IsRegister(const MCParsedAsmOperand &op) {
  return static_cast<const ZodiacOperand &>(op).isReg();
}
...
static bool MaybePredicatedInst(const OperandVector &Operands) {
  ...
  return StringSwitch<bool>(
             static_cast<const ZodiacOperand &>(*Operands[0]).getToken())
...
bool ZodiacAsmParser::parseInstruction(ParseInstructionInfo & /*Info*/,
  ...
  if (Lexer.is(AsmToken::EndOfStatement) && Name == "st" &&
    ...
    Operands.insert(Operands.begin(), ZodiacOperand::CreateToken("s", NameLoc));
    Operands.insert(Operands.begin() + 1,
                    ZodiacOperand::createImm(
  ...
  if (Lexer.is(AsmToken::EndOfStatement) && Name.starts_with("bt") &&
    ...
    Operands.insert(Operands.begin(), ZodiacOperand::CreateToken("bt", NameLoc));
  }
