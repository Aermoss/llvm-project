//===-- ZodiacMemAluCombiner.cpp - Pass to combine memory & ALU operations -===//
//
...
//
// The Zodiac ISA supports instructions where a load/store modifies the base
...
class ZodiacMemAluCombiner : public MachineFunctionPass {
public:
  static char ID;
  explicit ZodiacMemAluCombiner() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override {
    return "Zodiac load / store optimization pass";
  }
...
char ZodiacMemAluCombiner::ID = 0;

INITIALIZE_PASS(ZodiacMemAluCombiner, DEBUG_TYPE,
                "Zodiac memory ALU combiner pass", false, false)

namespace {
bool isSpls(uint16_t Opcode) { return Zodiac::splsIdempotent(Opcode) == Opcode; }
...
unsigned mergedOpcode(unsigned OldOpcode, bool ImmediateOffset) {
  ...
  case Zodiac::LDW_RR:
    if (ImmediateOffset)
      return Zodiac::LDW_RI;
    return Zodiac::LDW_RR;
  case Zodiac::LDHs_RI:
  case Zodiac::LDHs_RR:
    if (ImmediateOffset)
      return Zodiac::LDHs_RI;
    return Zodiac::LDHs_RR;
  case Zodiac::LDHz_RI:
  case Zodiac::LDHz_RR:
    if (ImmediateOffset)
      return Zodiac::LDHz_RI;
    return Zodiac::LDHz_RR;
  case Zodiac::LDBs_RI:
  case Zodiac::LDBs_RR:
    if (ImmediateOffset)
      return Zodiac::LDBs_RI;
    return Zodiac::LDBs_RR;
  case Zodiac::LDBz_RI:
  case Zodiac::LDBz_RR:
    if (ImmediateOffset)
      return Zodiac::LDBz_RI;
  ...
  case Zodiac::STB_RR:
    if (ImmediateOffset)
      return Zodiac::STB_RI;
    return Zodiac::STB_RR;
  case Zodiac::STH_RI:
  case Zodiac::STH_RR:
    if (ImmediateOffset)
      return Zodiac::STH_RI;
...
bool isZeroOperand(const MachineOperand &Op) {
  return ((Op.isReg() && Op.getReg() == Zodiac::R0) ||
...
void ZodiacMemAluCombiner::insertMergedInstruction(MachineBasicBlock *BB,
                                                  const MbbIterator &MemInstr,
...
bool isSuitableAluInstr(bool IsSpls, const MbbIterator &AluIter,
  ...
  if (Op2.isImm()) {
    ...
    if (AluIter->getOpcode() != Zodiac::ADD_I_LO)
      return false;

    if (Offset.isReg() && Offset.getReg() == Zodiac::R0)
      return true;
...
MbbIterator ZodiacMemAluCombiner::findClosestSuitableAluInstr(
...
bool ZodiacMemAluCombiner::combineMemAluInBasicBlock(MachineBasicBlock *BB) {
  bool Modified = false;
...
// machine function
bool ZodiacMemAluCombiner::runOnMachineFunction(MachineFunction &MF) {
  if (DisableMemAluCombiner)
    return false;

  TII = MF.getSubtarget<ZodiacSubtarget>().getInstrInfo();
  bool Modified = false;
...
FunctionPass *llvm::createZodiacMemAluCombinerPass() {
  return new ZodiacMemAluCombiner();
}
