...
static cl::opt<unsigned> SSThreshold(
    "zodiac-ssection-threshold", cl::Hidden,
    cl::desc("Small data and bss section threshold size (default=0)"),
    cl::init(0));

void ZodiacTargetObjectFile::Initialize(MCContext &Ctx,
...
// section.
bool ZodiacTargetObjectFile::isGlobalInSmallSection(
...
// Return true if this global address should be placed into small data/bss
// section.
bool ZodiacTargetObjectFile::isGlobalInSmallSection(const GlobalObject *GO,
                                                   const TargetMachine &TM,
...
// kind.
bool ZodiacTargetObjectFile::isGlobalInSmallSectionImpl(
    const GlobalObject *GO, const TargetMachine &TM) const {
...
MCSection *ZodiacTargetObjectFile::SelectSectionForGlobal(
...
bool ZodiacTargetObjectFile::isConstantInSmallSection(const DataLayout &DL,
                                                     const Constant *CN) const {
  return isInSmallSection(DL.getTypeAllocSize(CN->getType()));
}

MCSection *ZodiacTargetObjectFile::getSectionForConstant(
