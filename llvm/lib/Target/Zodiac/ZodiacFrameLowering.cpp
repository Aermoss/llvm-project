...
//===----------------------------------------------------------------------===//
//
// This file contains the Zodiac implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
// ADJDYNALLOC pseudo instructions with a Zodiac:ADDI with the
// maximum call frame size as the immediate.
void ZodiacFrameLowering::replaceAdjDynAllocPseudo(MachineFunction &MF) const {
  const ZodiacInstrInfo &LII = *STI.getInstrInfo();
  unsigned MaxCallFrameSize = MF.getFrameInfo().getMaxCallFrameSize();

  for (MachineBasicBlock &MBB : MF) {
    for (MachineInstr &MI : llvm::make_early_inc_range(MBB)) {
      if (MI.getOpcode() == Zodiac::ADJDYNALLOC) {
        DebugLoc DL = MI.getDebugLoc();
        Register Dst = MI.getOperand(0).getReg();
        Register Src = MI.getOperand(1).getReg();

        BuildMI(MBB, MI, DL, LII.get(Zodiac::ADD_I_LO), Dst)
            .addReg(Src)
...
void ZodiacFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  // Push old FP
  // st %fp,-4[*%sp]
  BuildMI(MBB, MBBI, DL, LII.get(Zodiac::SW_RI))
      .addReg(Zodiac::FP)
      .addReg(Zodiac::SP)
      .addImm(-4)
  ...
  // Generate new FP
  // add %sp,8,%fp
  BuildMI(MBB, MBBI, DL, LII.get(Zodiac::ADD_I_LO), Zodiac::FP)
      .addReg(Zodiac::SP)
      .addImm(8)
      .setMIFlag(MachineInstr::FrameSetup);
  ...
  // sub %sp,StackSize,%sp
  if (StackSize != 0) {
    BuildMI(MBB, MBBI, DL, LII.get(Zodiac::SUB_I_LO), Zodiac::SP)
        .addReg(Zodiac::SP)
        .addImm(StackSize)
        .setMIFlag(MachineInstr::FrameSetup);
  }
...
void ZodiacFrameLowering::emitEpilogue(MachineFunction & /*MF*/,
  ...
  BuildMI(MBB, MBBI, DL, LII.get(Zodiac::ADD_I_LO), Zodiac::SP)
      .addReg(Zodiac::FP)
      .addImm(0);

  // Restore the frame pointer from the stack.
  BuildMI(MBB, MBBI, DL, LII.get(Zodiac::LDW_RI), Zodiac::FP)
      .addReg(Zodiac::FP)
      .addImm(-8)
      .addImm(LPAC::ADD);
}
