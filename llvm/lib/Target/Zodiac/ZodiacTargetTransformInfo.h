//===-- ZodiacTargetTransformInfo.h - Zodiac specific TTI ---------*- C++ -*-===//
//
...
// Zodiac target machine. It uses the target's detailed information to
// provide more precise answers to certain TTI queries, while letting the
// target independent and default TTI implementations handle the rest.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_LIB_TARGET_ZODIAC_ZODIACTARGETTRANSFORMINFO_H
#define LLVM_LIB_TARGET_ZODIAC_ZODIACTARGETTRANSFORMINFO_H
...
namespace llvm {
class ZodiacTTIImpl final : public BasicTTIImplBase<ZodiacTTIImpl> {
  typedef BasicTTIImplBase<ZodiacTTIImpl> BaseT;
  typedef TargetTransformInfo TTI;
  friend BaseT;

  const ZodiacSubtarget *ST;
  const ZodiacTargetLowering *TLI;

  const ZodiacSubtarget *getST() const { return ST; }
  const ZodiacTargetLowering *getTLI() const { return TLI; }

public:
  explicit ZodiacTTIImpl(const ZodiacTargetMachine *TM, const Function &F)
...
#endif // LLVM_LIB_TARGET_ZODIAC_ZODIACTARGETTRANSFORMINFO_H
